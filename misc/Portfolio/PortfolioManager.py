...
class PortfolioManager:
    ...
    def getProfitsFrom(self, symbol):
        ...
        if not symbol in self.stocks:
            raise PortfolioError("You do not own shares of '%s'" % symbol)
    ...
    def getTotalShares(self, symbol):
        """
        Return the amount of shares under <symbol> in portfolio.
        """
        symbol = symbol.upper()
        ...
        total = 0
        for batch in self.stocks[symbol]:
            total += batch.amount
        return total
